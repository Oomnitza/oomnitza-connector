...
To run the local connector with Docker
    ...
    docker run --env_file .env oomnitza/oomnitza-connector:beta

    docker run --env_file .env oomnitza/oomnitza-connector:beta upload ldap

To run the local connector with Docker and a custom INI file (avoiding ENV vars):

    docker run -v $(pwd)/config.ini:/app/config.ini:ro oomnitza/oomnitza-connector:beta upload ldap
